...
| 🔭 | Người Quan Sát | `nguoiquansat.vn` | `nguoiquansat.vn/rss/trang-chu` (tài chính/chứng khoán/BĐS, audit 24/09/2026) |
...
- **Giao diện (v2.0 — "editorial newsroom", viết lại từ đầu 2026-09-16):** tham khảo Financial Times/Reuters/Bloomberg thay vì kiểu SaaS dashboard — nền trung tính, viền mỏng 1px, bóng gần như vô hình, không gradient/glassmorphism/emoji, không còn tô màu riêng theo từng nguồn. Font chính **Inter** (đã kiểm tra có bộ subset tiếng Việt riêng qua chính response CSS2 API của Google Fonts trước khi dùng — rút kinh nghiệm từ lỗi "Archivo Black" thiếu dấu tiếng Việt ở bản trước), font phụ **IBM Plex Mono** cho số liệu/nhãn. Trang chủ mở ra với **"All news"** — 1 dòng chảy tin hợp nhất từ toàn bộ 19 nguồn (mỗi dòng: giờ · nguồn · tiêu đề · nhãn issue nếu có), có ô tìm kiếm, bộ lọc theo nguồn/theo issue, và 2 chế độ sắp xếp "Mới nhất"/"Đang hot" — thay vì bảng Kanban làm màn hình mặc định như bản trước. Bảng Kanban cũ (mỗi nguồn 1 cột, đóng/mở bằng `<details>/<summary>` gốc, không cần JS) vẫn còn nguyên logic, chỉ lùi xuống thành mục phụ **"By source"**. Thanh điều hướng ngày (7 tab + dropdown "Ngày khác") lùi xuống mục **"Archive"** ở cuối trang. Xem chi tiết từng vùng giao diện ở [GIAO-DIEN.md](GIAO-DIEN.md).
...
Trên trang ngày **mới nhất** (không hiện ở các trang lưu trữ ngày cũ — xem lý do bên dưới), ngay dưới "Today overview" có 1 lưới 2 cột (desktop) xếp hạng 5 "issue" (sự kiện cụ thể) được nhiều báo cùng đưa tin nhất **trong ngày hôm nay** (theo giờ Việt Nam), để nắm nhanh tình hình chung mà không cần đọc hết từng nguồn. Mỗi thẻ issue gọn (~180-220px khi thu gọn: hạng, tiêu đề, HotScore, số bài/nguồn, 2 dòng "Vì sao hot"), bấm vào để xem chi tiết đầy đủ (4 thành phần HotScore, biểu đồ thanh ngang mức độ đưa tin theo từng nguồn, toàn bộ bài liên quan). Logic tính toán là bản viết lại từ đầu của panel "Sự kiện nổi bật" cũ, theo đặc tả `PROJECT_SPEC_V3_ISSUE_INTELLIGENCE.md`; phần giao diện được thiết kế lại lần nữa theo đặc tả v2.0 "editorial newsroom" ở trên.
...
- **HotScore** — mỗi thành phần chuẩn hoá riêng 0-100 theo giá trị cao nhất trong đợt tính hiện tại (và hiển thị được từng thành phần để dò lỗi, không chỉ điểm tổng), rồi nhân trọng số:
  - 40% Số bài đề cập (`volume_score`)
  - 25% Số nguồn đề cập (`source_score`)
  - 20% Tốc độ xuất hiện — số bài / số giờ kể từ lần đầu thấy issue đó (`velocity_score`)
  - 15% Mức độ mới/tăng tốc — so nhịp ra bài ở 1/4 thời gian gần nhất so với trước đó, cộng thêm số nguồn mới chỉ xuất hiện gần đây (`novelty_score`)
  - **Giới hạn thật đã biết:** trọng số 25% cho đa dạng nguồn là 1 "lực đối trọng" chứ không phải "phủ quyết" — 1 nguồn đăng đủ nhiều tin gần giống nhau vẫn có thể vượt điểm 1 issue thật sự đa nguồn nếu chênh lệch số bài đủ lớn (xem docstring `_score_all()` trong `web/issues.py` và test tương ứng trong `tests/test_issues.py`).
...
- **Click để xem toàn bộ bài liên quan:** mỗi thẻ issue là 1 `<details>` — click vào để mở phần chi tiết: 4 thành phần HotScore, giờ thấy đầu tiên/gần nhất, biểu đồ thanh ngang số bài theo từng nguồn (`_source_coverage_html`, không dùng pie chart theo đúng đặc tả v2.0), và toàn bộ bài viết thuộc issue đó (không chỉ phần tóm tắt trên thẻ thu gọn). Trong "All news", các dòng tin thuộc 1 issue có thêm nhãn nhỏ link thẳng tới đúng thẻ issue đó (`href="#issue-{id}"`), tự mở ra nhờ CSS `:target` — không cần JS.
- **Vì sao không hiện ở trang ngày cũ:** HotScore tính theo "hiện tại" (thời điểm build site), gắn nó vào 1 trang lưu trữ của ngày trong quá khứ sẽ gây hiểu lầm là phản ánh đúng lúc đó. `build_site` chỉ tính 1 lần và chỉ gắn vào đúng trang ngày mới nhất.
- **Vị trí panel:** nằm ngay dưới "Today overview" và phía trên "All news" — theo đúng đặc tả giao diện v2.0 (khác với bản trước, từng đặt dưới bảng Kanban theo yêu cầu cũ; bảng Kanban giờ đã lùi xuống thành mục phụ "By source" nên xung đột vị trí trước đó không còn nữa).
...
- **Xuất dữ liệu** ([web/exports.py](web/exports.py)), ghi cạnh các trang HTML mỗi lần build: `issues.json` (Top Issues + 4 thành phần HotScore), `stats.json` (`daily_stats` + `issue_history`), `feed.xml` (RSS 100 bài mới nhất, URL kênh lấy từ `NEWS_SITE_URL`).
...
**Trên web:** tab `BRANDS` (cảnh báo hiện hành, 2 bảng share of voice, số tin theo 14 ngày, tin tiêu cực gần đây), mỗi dòng "All news" có chấm màu sắc thái + nhãn thương hiệu, bộ lọc theo thương hiệu, và xuất `brands.json`.
