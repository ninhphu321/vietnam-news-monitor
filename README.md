...
### 🔥 Panel "Top 5 Issues hôm nay" (Issue Intelligence V3)

Trên trang ngày **mới nhất** (không hiện ở các trang lưu trữ ngày cũ — xem lý do bên dưới), có 1 tab gấp/mở được nằm **dưới** bảng Kanban, xếp hạng 5 "issue" (sự kiện cụ thể) được nhiều báo cùng đưa tin nhất **trong ngày hôm nay** (theo giờ Việt Nam), để nắm nhanh tình hình chung mà không cần đọc hết từng nguồn. Đây là bản viết lại từ đầu của panel "Sự kiện nổi bật" cũ, theo đặc tả `PROJECT_SPEC_V3_ISSUE_INTELLIGENCE.md`.
...
- **Click để xem toàn bộ bài liên quan:** mỗi thẻ issue là 1 `<details>` lồng bên trong panel — click vào để mở ra danh sách đầy đủ tất cả bài viết thuộc issue đó (không chỉ 3 bài đại diện hiển thị mặc định), kèm giờ thấy đầu tiên/gần nhất và danh sách nguồn.
...
- **Vị trí panel:** nằm dưới bảng Kanban thay vì phía trên (khác với gợi ý mặc định trong đặc tả V3) — theo yêu cầu trực tiếp của người dùng để ưu tiên xem tin theo nguồn trước.
