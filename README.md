...
### 🔥 Panel "Sự kiện nổi bật" (top 5, HotScore)

Trên trang ngày **mới nhất** (không hiện ở các trang lưu trữ ngày cũ — xem lý do bên dưới), có 1 bảng xếp hạng 5 chủ đề đang được nhiều báo cùng đưa tin nhất, để nắm nhanh tình hình chung mà không cần đọc hết từng nguồn.

- Module: [web/trending.py](web/trending.py). **Không dùng AI/embedding** (đúng tinh thần "không sentiment/event clustering" đã nêu ở mục Giới hạn đã biết) — phát hiện "sự kiện" bằng cách tìm cụm 1-3 từ xuất hiện lặp lại trong tiêu đề của **nhiều bài, từ nhiều nguồn khác nhau**, gần đây (cửa sổ 48 giờ). Đây là suy luận theo từ khóa, không phải hiểu nghĩa thật sự — 2 bài viết về cùng 1 sự kiện nhưng đặt tít hoàn toàn khác từ ngữ sẽ không được gộp. Hoạt động khá tốt cho tin tài chính tiếng Việt vì tên công ty/ngân hàng/người thường được viết nguyên văn giống nhau giữa các báo, không diễn giải lại.
- **HotScore** — đúng công thức yêu cầu, mỗi thành phần chuẩn hoá 0-1 theo giá trị cao nhất trong đợt tính hiện tại rồi nhân trọng số:
  - 40% Số bài đề cập
  - 25% Số nguồn đề cập
  - 20% Tốc độ xuất hiện (số bài / số giờ kể từ lần đầu thấy cụm từ đó)
  - 15% Mức độ mới/tăng tốc (so nhịp ra bài ở 1/4 thời gian gần nhất so với trước đó — một chủ đề vừa bùng lên tính điểm cao hơn 1 chủ đề đã rải đều cả 48 giờ dù tổng số bài bằng nhau)
- **Lọc nhiễu:** loại cụm từ chỉ xuất hiện ở 1 nguồn (chưa đủ "nhiều báo cùng đưa"), loại cụm từ quá phổ biến (ngưỡng document-frequency ~3% tổng số bài trong cửa sổ — nếu không sẽ bị các từ chung chung như "ngân hàng", "chứng khoán", "Việt Nam" áp đảo top 5 mỗi ngày dù chẳng phải sự kiện gì cụ thể — phát hiện được bằng cách chạy thử trên dữ liệu thật ngày 2026-09-16), và có 1 danh sách `_GENERIC_PHRASES`/stopword mở rộng loại thêm các cụm/động từ tiêu đề quá thông dụng ("đề xuất", "tiếp tục", "hỗ trợ"...) — cùng kiểu "danh sách tự tay chọn, không AI" như `HOT_KEYWORDS` trong `telegram.py`, và cũng cần tinh chỉnh dần theo thời gian như vậy, không phải giải pháp hoàn hảo.
